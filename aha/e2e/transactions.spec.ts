...
async function ensureTxType(page: Page): Promise<void> {
  await gotoAndWaitForTable(page, "/transaction-types", "/api/transaction-types");
...
test.describe("取引管理", () => {
  test.beforeEach(async ({ page }) => {
    await gotoAndWaitForTable(page, "/transactions", "/api/transactions");
  });
  ...
  test("取引を新規登録できる（品目タイプアヘッド経由）", async ({ page }) => {
    ...
    // 取引区分を選択
    await form.getByLabel("取引区分").selectOption({ index: 1 });
  ...
  test("日付フィルタが機能する", async ({ page }) => {
    const dateFrom = page.getByLabel("開始日");
    const dateTo = page.getByLabel("終了日");
