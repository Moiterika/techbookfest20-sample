...
function genHandler(fm: FeatureMapping, config: EntityConfigDef | null, columns: ColumnDef[] = [], entity?: EntityDef): string {
  ...
  const dynamicSelectCols = dataCols.filter(c => c.type === "select" && (!c.options || c.options.length === 0));
  const hasDynamicSelect = dynamicSelectCols.length > 0;
  ...
  if (hasDynamicSelect) {
    for (const col of dynamicSelectCols) {
      const refTable = col.key.replace(/ID$/, "");
      const paramName = `${toPascalCase(col.key)}Options`;
      lines.push(`\t${paramName}, err := FetchSelectOptions(h.DB, "${refTable}")`);
      lines.push(`\tif err != nil {`);
      lines.push(`\t\thttp.Error(w, err.Error(), http.StatusInternalServerError)`);
      lines.push(`\t\treturn`);
      lines.push(`\t}`);
    }
    const args = dynamicSelectCols.map(c => `${toPascalCase(c.key)}Options`).join(", ");
    lines.push(`\tRenderPage${JP}(${args}).Render(r.Context(), w)`);
  } else {
  ...
  if (hasDynamicSelect) {
    for (const col of dynamicSelectCols) {
      const refTable = col.key.replace(/ID$/, "");
      const paramName = `${toPascalCase(col.key)}Options`;
      lines.push(`\t${paramName}, optErr := FetchSelectOptions(h.DB, "${refTable}")`);
      lines.push(`\tif optErr != nil {`);
      lines.push(`\t\thttp.Error(w, optErr.Error(), http.StatusInternalServerError)`);
      lines.push(`\t\treturn`);
      lines.push(`\t}`);
    }
    const args = dynamicSelectCols.map(c => `${toPascalCase(c.key)}Options`).join(", ");
    lines.push(`\tRenderEditRow${JP}(item, ${args}).Render(r.Context(), w)`);
  } else {
  ...
  if (exportCols.length > 0 && entity) {
    ...
    lines.push(`\tw.Header().Set("Content-Disposition", "attachment; filename*=UTF-8''"+url.PathEscape(filename))`);
...
function genViewsTempl(entity: EntityDef, fm: FeatureMapping, columns: ColumnDef[], config: EntityConfigDef): string {
  ...
  // 動的 select カラム（options が空）を検出
  const dynamicSelectCols = dataCols.filter(c => c.type === "select" && (!c.options || c.options.length === 0));
  const hasDynamicSelect = dynamicSelectCols.length > 0;
  const pageParams = hasDynamicSelect ? dynamicSelectCols.map(c => `${toPascalCase(c.key)}Options []SelectOption`).join(", ") : "";
  const hasDateCol = dataCols.some(c => c.type === "date");
  ...
  lines.push("import (");
  lines.push('\t"fmt"');
  if (hasDateCol) lines.push('\t"time"');
  lines.push('\t"aha-goth/internal/ui"');
  lines.push(")");
  lines.push("");

  // ── Page ──
  lines.push(`templ RenderPage${JP}(${pageParams}) {`);
  ...
  lines.push(`\t\t\t\t\t<form hx-post="${apiPath}" hx-target="#${config.bodyTargetId}" hx-swap="innerHTML" @htmx:after-request="if($event.detail.successful && $event.detail.elt === $el) { $el.reset(); open = false }" class="flex flex-col gap-4">`);
  ...
  // ── EditRow ──
  const editRowParams = hasDynamicSelect
    ? `item Response${JP}, ${pageParams}`
    : `item Response${JP}`;
  lines.push(`templ RenderEditRow${JP}(${editRowParams}) {`);
  ...
  lines.push(`\t\t\t\t<form id={ fmt.Sprintf("edit-form-%d", item.Id) } hx-put={ fmt.Sprintf("${apiPath}/%d", item.Id) } hx-target={ fmt.Sprintf("#${config.idPrefix}-%d", item.Id) } hx-swap="outerHTML"><button type="submit" class="${TW.buttonPrimary}">保存</button></form>`);
  lines.push(`\t\t\t\t<button class="${TW.buttonSecondary}" hx-get="${apiPath}" hx-target="#${config.bodyTargetId}" hx-swap="innerHTML">キャンセル</button>`);
  lines.push(`\t\t\t</div>`);
  lines.push("\t\t</td>");
  lines.push("\t</tr>");
  lines.push("}");
...
function genStyledEditCell(col: ColumnDef, apiPath: string, idPrefix: string, entity: EntityDef, TW: { tdCell: string; inputStyleSm: string }): string {
  ...
  switch (col.type) {
    ...
    case "select": {
      ...
      // 動的 options
      const paramName = `${toPascalCase(col.key)}Options`;
      return `<td${cls}><select name="${col.key}" form={ fmt.Sprintf("edit-form-%d", item.Id) }${inputCls}>\n\t\t\tfor _, opt := range ${paramName} {\n\t\t\t\t<option value={ opt.Value } if fmt.Sprintf("%v", item.${goField}) == opt.Value { selected }>{ opt.Label }</option>\n\t\t\t}\n\t\t</select></td>`;
    }
    case "computed":
      return `<td${cls}>{ fmt.Sprintf("%d", item.${goField}) }</td>`;
    case "readonlyLookup":
...
function genStyledFormField(col: ColumnDef, TW: { labelStyle: string; inputStyle: string }): string {
  ...
  switch (col.type) {
    ...
    case "date":
      return `<label${lblCls}>${col.label}<input type="date" name="${col.key}" value={ time.Now().Format("2006-01-02") }${col.required ? " required" : ""}${inputCls}/></label>`;
    case "select": {
      if (col.options && col.options.length > 0) {
      ...
      // 動的 options: templ for ループで描画
      const paramName = `${toPascalCase(col.key)}Options`;
      return `<label${lblCls}>${col.label}<select name="${col.key}"${col.required ? " required" : ""}${inputCls}>\n\t\t\t\t\t\t\tfor _, opt := range ${paramName} {\n\t\t\t\t\t\t\t\t<option value={ opt.Value }>{ opt.Label }</option>\n\t\t\t\t\t\t\t}\n\t\t\t\t\t\t</select></label>`;
    }
    case "barcode":
    ...
    case "itemCode": {
      ...
      return `<label${lblCls}><span>${col.label}</span>@RenderTypeahead品目("${valueName}", 0, "", "", false, false, "")</label>`;
    }
    default: // text
...
function genHeaderBodyViewsTempl(parentEntity: EntityDef, fm: FeatureMapping, hbDef: HeaderBodyDef): string {
  ...
  // ── NewPage (create form) ──
  const newPageTitle = `${JP}登録`;
  lines.push(`templ RenderNewPage${JP}() {`);
  lines.push(`\t@ui.Base("${newPageTitle}") {`);
  lines.push(`\t\t<main class="${TW.pageContainer}">`);
  lines.push(`\t\t\t@ui.Nav("${navKey(JP)}")`);
  lines.push(`\t\t\t<h1 class="${TW.pageTitle}">${newPageTitle}</h1>`);
  // form
