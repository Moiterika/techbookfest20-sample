...
function genService(fm: FeatureMapping): string {
  ...
  lines.push(`type Handler${JP} struct { DB *gorp.DbMap }`);
  lines.push("");
  lines.push(`func NewHandler${JP}(db *gorp.DbMap) *Handler${JP} {`);
...
function genViewsTempl(entity: EntityDef, fm: FeatureMapping, columns: ColumnDef[], config: EntityConfigDef): string {
  ...
  // ツールバー
  lines.push(`\t\t\t\t<div class="flex justify-between items-center mb-6 flex-wrap gap-4" x-data="{ hasSelection: false, updateSelection() { const tbody = document.getElementById('${config.bodyTargetId}'); this.hasSelection = tbody ? tbody.querySelectorAll('input[name=\\'rowSelect\\']:checked').length > 0 : false; } }" @change.window="updateSelection()" @htmx:after-swap.window="setTimeout(() => updateSelection(), 0)">`);
...
func Validate登録${JP}(_ context.Context, _ *gorp.DbMap, _ 作成Input${JP}) error {
\treturn nil
}

func Validate更新${JP}(_ context.Context, _ *gorp.DbMap, _ 更新Input${JP}) error {
\treturn nil
}
...
function genHeaderBodyHandler(fm: FeatureMapping, config: EntityConfigDef): string {
  const JP = fm.jpName;
  const apiPath = config.baseUrl;
...
function genHeaderBodyViewsTempl(parentEntity: EntityDef, fm: FeatureMapping, hbDef: HeaderBodyDef): string {
  ...
  lines.push(`\t\t<main class="${TW.pageContainer}">`);
  lines.push(`\t\t\t<nav class="mb-4">`);
  lines.push(`\t\t\t\t<a href="/${JP}" class="text-primary no-underline hover:underline">← ${JP}一覧</a>`);
  lines.push(`\t\t\t</nav>`);
...
func Validate登録${JP}(_ context.Context, _ *gorp.DbMap, _ 作成Input${JP}WithLines) error {
\treturn nil
}

func Validate更新${JP}(_ context.Context, _ *gorp.DbMap, _ 更新Input${JP}WithLines) error {
\treturn nil
}
