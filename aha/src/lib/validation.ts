...
export const 取引区分作成Schema = z.object({
  code: z.string().min(1, "取引区分コードは必須です"),
  name: z.string().min(1, "取引区分名称は必須です"),
...
export const 取引作成Schema = z.object({
  ...
  transactionTypeId: z.coerce.number().positive("取引区分を選択してください"),
  itemId: z.coerce.number().positive("品目を選択してください"),
...
export const BOM明細Schema = z.object({
  ...
  refBomCode: z.string().nullable().optional(),
  refBomVersion: z.string().nullable().optional(),
});

export const BOM作成Schema = z.object({
  code: z.string().min(1, "BOMコードは必須です"),
  version: z.string().min(1, "版は必須です"),
  name: z.string().min(1, "名称は必須です"),
  lines: z.array(BOM明細Schema),
});
