...
| DB テーブル名 | **MUST** 英語 snake_case 複数形 | `users`, `order_lines` |
| DB カラム名 | **MUST** 英語 snake_case | `customer_name` |
| TypeScript 型名（DB由来） | **MUST** 英語 PascalCase | `User`, `OrderLine` |
| Feature 内ファイル名 | **MUST** 日本語（操作名＋エンティティ名） | `ユーザー登録Command.ts` |
| Feature 内サブディレクトリ | **MUST** 日本語（操作カテゴリ） | `登録/`, `更新/`, `削除/`, `一覧/` |
...
**日本語と英語の混在は禁止**。上記の表で言語が決められた箇所は必ずそれに従うこと。
...
│   ├── 一覧/ユーザー一覧Query.ts          # 一覧取得クエリ
│   ├── 登録/
│   │   ├── ユーザー登録Args.ts           # DB 投入用の型
│   │   ├── ユーザー登録Command.ts        # 登録コマンド
│   │   ├── ユーザー登録Validate.ts       # カスタムバリデーション
│   │   └── ユーザー作成Mapper.ts         # 入力 → Args 変換
│   ├── 更新/
│   │   ├── ユーザー更新Args.ts
│   │   ├── ユーザー更新Command.ts
│   │   ├── ユーザー更新Validate.ts
│   │   └── ユーザー更新Mapper.ts
│   └── 削除/
│       ├── ユーザー削除Command.ts         # 単一削除
│       └── ユーザー一括削除Command.ts     # 一括削除
├── pages/
│   ├── ユーザー.astro                    # 一覧ページ
│   └── api/ユーザー/
...
export const ユーザーカラム: Column[] = [
  { key: "code", label: "ユーザーコード", required: true, placeholder: "例: USR-001" },
  { key: "name", label: "氏名", required: true, placeholder: "例: 山田太郎" },
...
export type ユーザーResponse = User;
```

**`src/features/ユーザー/登録/ユーザー登録Args.ts`**:
```typescript
export type ユーザー登録Args = {
  code: string;
  name: string;
  email: string | null;
...
**`src/features/ユーザー/登録/ユーザー作成Mapper.ts`**:
```typescript
import type { ユーザー作成入力 } from "../../../lib/validation";
import type { ユーザー登録Args } from "./ユーザー登録Args";

export const ユーザー作成Mapper: (input: ユーザー作成入力) => ユーザー登録Args = (input) => ({
  code: input.code,
  name: input.name,
...
**`src/features/ユーザー/登録/ユーザー登録Validate.ts`**:
```typescript
import type { ユーザー作成入力 } from "../../../lib/validation";

export const ユーザー登録Validate: (input: ユーザー作成入力) => Promise<void> = async (_input) => {
  // 必要に応じてコード重複チェック等を追加
};
```

**`src/features/ユーザー/登録/ユーザー登録Command.ts`**:
```typescript
import { GenericCommand } from "../../core/generic-command";
import { ユーザー作成Schema, type ユーザー作成入力 } from "../../../lib/validation";
import type { ユーザー登録Args } from "./ユーザー登録Args";
import { ユーザー登録Validate } from "./ユーザー登録Validate";
import { ユーザー作成Mapper } from "./ユーザー作成Mapper";
import { db } from "../../../db";
import { users } from "../../../db/schema";

export const ユーザー登録Command = new GenericCommand<ユーザー作成入力, ユーザー登録Args, void>({
  schema: ユーザー作成Schema,
  validate: ユーザー登録Validate,
  mapper: ユーザー作成Mapper,
  command: async (args) => {
    await db.insert(users).values(args);
  },
});
```

**`src/features/ユーザー/一覧/ユーザー一覧Query.ts`**:
```typescript
...
export const ユーザー一覧Query = new GenericQuery<ユーザー一覧入力, ユーザー一覧Result>({
  ...
  query: async (input) => {
    ...
    if (input.q) {
      conditions.push(
        or(ilike(users.code, `%${input.q}%`), ilike(users.name, `%${input.q}%`)),
      );
    }
    if (input.department) {
      conditions.push(ilike(users.department, `%${input.department}%`));
    }
    const searchFilter = conditions.length > 0 ? and(...conditions) : undefined;

    const [{ total }] = await db.select({ total: count() }).from(users).where(searchFilter);
    ...
    if (input.department) extraParams.department = input.department;
...
export { ユーザー一覧Query } from "./一覧/ユーザー一覧Query";
export { ユーザー登録Command } from "./登録/ユーザー登録Command";
export { ユーザー一括削除Command } from "./削除/ユーザー一括削除Command";
...
import { ユーザー一覧Query, ユーザー登録Command, ユーザー一括削除Command } from "../../../features/ユーザー";
...
export const DELETE: APIRoute = async ({ request }) => {
  ...
  await ユーザー一括削除Command.execute({ ids: numIds });
...
```
src/
├── db/schema.ts                            # ← テーブル定義を追加 (orders + order_lines)
├── lib/validation.ts                       # ← Zod スキーマを追加
├── features/注文/
│   ├── config.ts                           # HeaderBodyConfig 定義
│   ├── index.ts                            # 公開エクスポート
│   ├── 注文Response.ts                     # レスポンス型
│   ├── 一覧/注文一覧Query.ts               # ヘッダー一覧クエリ
│   ├── 登録/
│   │   ├── 注文登録Args.ts
│   │   ├── 注文登録Command.ts              # ヘッダー＋明細を一括登録
│   │   ├── 注文登録Validate.ts
│   │   └── 注文作成Mapper.ts
│   ├── 更新/
│   │   ├── 注文更新Args.ts
│   │   ├── 注文更新Command.ts              # ヘッダー更新 + 明細差し替え
│   │   ├── 注文更新Validate.ts
│   │   └── 注文更新Mapper.ts
│   └── 削除/
│       ├── 注文削除Command.ts
...
export const 注文ヘッダーボディ: HeaderBodyConfig = {
  ...
  displayNameKey: "code",
  deleteConfirmTemplate: "注文「{code}」を削除しますか？",
  formTitle: "新規注文登録",
  emptyMessage: "注文がありません",

  // ヘッダー部のカラム定義（一覧テーブルのカラムにもなる）
  headerColumns: [
    { key: "code", label: "注文番号", required: true, placeholder: "例: ORD-001" },
    { key: "date", label: "注文日", type: "date", required: true },
    { key: "customerName", label: "顧客名", required: true, placeholder: "例: 山田商事" },
  ],
  ...
  children: [
    {
      ...
      columns: [
        ...
        { key: "quantity", label: "数量", type: "number", min: 0, placeholder: "0" },
        {
          key: "unit",
          label: "単位",
          type: "select",
...
import { pageContainer, pageTitle } from "../../styles/common";
import { db } from "../../db";
import { orders, orderLines, items } from "../../db/schema";
...
[注文/new.astro]  新規登録ページ
    ...
    └── HeaderBodyForm (mode="create")
         ...
         送信 → POST /api/注文
              → 注文登録Command (ヘッダー INSERT → 明細 INSERT)
              → リダイレクト /注文/[id]?saved
...
[注文/[id].astro]  編集ページ
    ...
    └── HeaderBodyForm (mode="edit")
         送信 → PUT /api/注文/[id]
              → 注文更新Command (ヘッダー UPDATE + 明細 DELETE & INSERT)
              → リダイレクト /注文/[id]?saved
```
