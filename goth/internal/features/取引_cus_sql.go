...
func Execute一覧SQL取引(ctx context.Context, db *sql.DB, input 一覧Input取引) (ListResult取引, error) {
	...
	rows, err := db.QueryContext(ctx,
		fmt.Sprintf(`SELECT id, "日付", "取引区分ID", "品目ID", "単価", "数量", "金額", created_at, updated_at
		 FROM "取引" %s ORDER BY id DESC LIMIT $%d OFFSET $%d`, where, offset+1, offset+2),
...
func Execute登録SQL取引(ctx context.Context, db *sql.DB, input 作成Input取引) (Response取引, error) {
	...
	err := db.QueryRowContext(ctx,
		`INSERT INTO "取引" ("日付", "取引区分ID", "品目ID", "単価", "数量", "金額", created_at, updated_at)
		 VALUES ($1, $2, $3, $4, $5, $6, $7, $8)
		 RETURNING id, "日付", "取引区分ID", "品目ID", "単価", "数量", "金額", created_at, updated_at`,
